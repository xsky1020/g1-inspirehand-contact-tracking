...
if __name__ == "__main__":
    ...
    with open(f"{LEGGED_GYM_ROOT_DIR}/deploy/deploy_mujoco/configs/{config_file}", "r", encoding="utf-8") as f:
        ...
        policy_path = config["policy_path"].replace("{LEGGED_GYM_ROOT_DIR}", LEGGED_GYM_ROOT_DIR) + "Squat.pt"
    ...
    pick_time = 5.2   # seconds
    with mujoco.viewer.launch_passive(m, d) as viewer:
        # Close the viewer automatically after simulation_duration wall-seconds.
        start = time.time()
        rel_pose_object = [0., 0., 0.5860, 1., 0., 0., 0.] # Get this
    
        counter = 0
    
        d.qpos[7:] = default_angles
        d.qvel[6:] = 0.
        d.qpos[:3] = np.array([0., 0., 0.813])
    
        d.qpos[3:7] = np.array([0.9971888, 0, 0.0749297, 0.]) 
        d.qvel[:6] = 0.
        mujoco.mj_step(m, d)
        run_stand_still(5.)
        counter = 0.
