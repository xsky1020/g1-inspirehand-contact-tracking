...
if __name__ == "__main__":
    ...
    with open(f"{LEGGED_GYM_ROOT_DIR}/deploy/deploy_mujoco/configs/{config_file}", "r", encoding="utf-8") as f:
        ...
        policy_path = config["policy_path"].replace("{LEGGED_GYM_ROOT_DIR}", LEGGED_GYM_ROOT_DIR) + "Punch_UB.pt"
    ...
    with mujoco.viewer.launch_passive(m, d) as viewer:
        ...
        object_pose = np.array(rel_pose_object) + np.array([0.13, 0., 0.753, 0., 0., 0., 0.])
        ...
        while viewer.is_running() :
            ...
            if counter*simulation_dt > 7. :
                counter = 0
                rel_pose_object[0] = np.random.uniform(0.435, 0.485) # Get this
                rel_pose_object[1] = np.random.uniform(-0.27, -0.04)
                rel_pose_object[2] = np.random.uniform(0.17, 0.44)
                object_pose = np.array(rel_pose_object) + np.array([0.13, 0., 0.753, 0., 0., 0., 0.])
                body_id = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_BODY, "object")
                m.body_pos[body_id] = object_pose[:3]
                m.body_quat[body_id] = object_pose[3:7]
            ...
            if counter % control_decimation == 0:
                ...
                right_hand_state = (count > punch_time) * 1.0
                right_hand_state_1 = (count < punch_time + 0.7) * (count > punch_time - 0.7) * 1.0
                obs[:3] = gravity_orientation
                obs[3:3+len(active_joint_ids)] = qj
                obs[3+len(active_joint_ids):3+2*len(active_joint_ids)] = dqj
                # print(num_actions, action)
