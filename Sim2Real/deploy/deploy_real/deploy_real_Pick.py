...
POLICY_TIME = 15.0
TARGET_LOC = [0.41, -0.2, 0.213, 1., 0., 0., 0.]
...
class ControllerPick(Controller):
    def run(self):
        self.counter += 1
        t1 = time.time()
...
if __name__ == "__main__":
    ...
    hands.grasp("left")
    hands.release("right")
    controller = ControllerPick(config, hands, policy_name="Pick")
    ...
    for i in range(5):
        controller_done = controller.run(exec_cmd=False)
