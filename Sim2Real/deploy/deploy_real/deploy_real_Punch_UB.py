...
CLOSED_CONFIG = {
    ...
    "LeftRing" : 0.,
    "LeftPinky" : 0.,
    "LeftThumbRotation" : 0.5,        
    "RightThumbBend" : 0.,
    "RightIndex" : 0.,
    "RightMiddle" : 0.,
    "RightRing" : 0.,
    "RightPinky" : 0.,
    "RightThumbRotation" : 0.5        
}
...
PUNCH_TIME = 3.5
POLICY_TIME = 10.0
TARGET_LOC = [0.46, -0.1, 0.42, 1., 0., 0., 0.]
obj = "circle target with cross"
...
def get_target_point(pipeline):
    ...
    while True:
        ...
        if rgb is not None and skip_frames <= 0:
            # Detect the object
            ...
            if boxes is not None and len(boxes) > 0:
                ...
                if i_max != -1 :    
                    ...
                    if mid_x != -1 and mid_y != -1:
                        ...
                        y_center = min(-0.04,max(-0.27,y_center))
                        z_center = min(0.44,max(0.17,z_center))
...
class ControllerPunch(Controller):
    def run(self):
        self.counter += 1
        t1 = time.time()
...
if __name__ == "__main__":
    ...
    controller = ControllerPunch(config, hands, policy_name="Punch_UB")
