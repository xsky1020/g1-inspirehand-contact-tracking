...
class ConfigNew:
    def __init__(self, file_path) -> None:
        with open(file_path, "r") as f:
            ...
            self.inactive_idx = config["inactive_idx"]
            self.inactive_kps = config["inactive_kps"]
            self.inactive_kds = config["inactive_kds"]
            self.inactive_target = np.array(config["inactive_target"], dtype=np.float32)

            self.active_idx = config["active_idx"]
            self.active_kps = config["active_kps"]
            self.active_kds = config["active_kds"]
            self.active_target = np.array(config["active_target"], dtype=np.float32)
